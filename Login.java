...
class Log extends Base implements ActionListener 
{
	...
	String arr[] = { "Receptionist", "Employee", "Doctor" };
	boolean flag;
	String text,pwd;
	Log() 
	{
					...
					if (frmvalidate()) 
					{
						...
						else if (comboBox.getSelectedItem().equals("Receptionist"))
						{
							...
							new java.util.Timer().schedule(new TimerTask() 
							{
								...
								{
									RegisterOperation ro = new RegisterOperation();
									ResultSet rs = ro.rec_sec_record((txt_userid.getText()));
									try 
									{
										if (rs.next())
										{
											RecPortal rf = new RecPortal();
											rf.idlabel.setText(rs.getString(1));
											rf.namelabel.setText(rs.getString(2));
										}
									}
									catch (SQLException e1)
									{
						...
						else if (comboBox.getSelectedItem().equals("Doctor")) 
						{
							...
							if (dl.doc_loginValidate()) 
							{
								...
								{
									RegisterOperation ro = new RegisterOperation();
									ResultSet rs = ro.doc_sec_record((txt_userid.getText()));
									try 
									{
										if (rs.next())
										{
											DocPortal doc = new DocPortal();
											doc.idlabel.setText(rs.getString(1));
											doc.namelabel.setText(rs.getString(2));
										}
									}
									catch (SQLException e1)
									{
						...
						else if (comboBox.getSelectedItem().equals("Employee"))
						{
							...
							new java.util.Timer().schedule(new TimerTask() 
							{
								...
								{
									RegisterOperation ro = new RegisterOperation();
									ResultSet rs = ro.emp_sec_record((txt_userid.getText()));
									try 
									{
										if (rs.next())
										{
											EmpPortal emp = new EmpPortal();
											emp.idlabel.setText(rs.getString(1));
											emp.namelabel.setText(rs.getString(2));
										}
									}
									catch (SQLException e1)
									{
