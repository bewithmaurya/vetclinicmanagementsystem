...
class EmpPortal extends Base implements ActionListener
{
	public JFrame frm;
	public JLabel Fullimg, userlabel, namelabel, idlabel, lblupdateprofile, adminlabel, lblVeterinaryManagementSystem, lblAdminPortal, barlabel;
	public JButton appbtn, btnWork, empbtn, drtimebtn, drugbtn, vaccbtn, servbtn, app1btn, btnlok, btnlogof, btnExit;
	...
	EmpPortal() 
	{
		...
		Image adminimg = new ImageIcon(this.getClass().getResource("/images/emp_icon.png")).getImage();
		...
		lblupdateprofile.addMouseListener(new MouseAdapter() 
		{
			...
			public void mouseClicked(MouseEvent e) 
			{
				...
				ResultSet rs = ro.emp_sec_recordUpdate((idlabel.getText()));
				try 
				{
					if (rs.next()) 
					{
						EmpUpdate ru = new EmpUpdate();
		...
		barlabel = new JLabel("");
		Image barimg = new ImageIcon(this.getClass().getResource("/images/emp_bar.png")).getImage();
		barlabel.setIcon(new ImageIcon(barimg));
		barlabel.setBounds(91, 19, 1293, 102);
		frm.add(barlabel);

		btnWork = new JButton();
		Image recimg = new ImageIcon(this.getClass().getResource("/images/workperform.png")).getImage();
		btnWork.setIcon(new ImageIcon(recimg));
		btnWork.setFont(new Font("Bookman Old Style", Font.BOLD, 20));
		btnWork.setBounds(30, 480, 370, 60);
		btnWork.addActionListener(this);
		frm.add(btnWork);

		Fullimg = new JLabel();
		Image img5 = new ImageIcon(this.getClass().getResource("/images/emp_img.png")).getImage();
	...
	{
		if (e.getSource().equals(btnWork))
		{
			String curdate = new SimpleDateFormat("dd/MMM/yyyy").format(Calendar.getInstance().getTime());
			EmpWork ew = new EmpWork();
			ew.lblEmployeeName.setText(namelabel.getText());
			ew.ShowTable(curdate, namelabel.getText());
		} 
