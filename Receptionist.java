...
class Rec extends ValidateForm implements ActionListener 
{
	public JFrame frm;
	...
	public JLabel lblId, lbluser, lblsAnswer, lblpass, lblconfpass, lblSearch, lblVeterinaryManagementSystem,lblRegister, barlabel, lblsQueston, lblEmailId, lblName, lblDOB, lblGender, lblFull_Address, lblMob_No,lblimg;
	...
	Rec()
	{
		...
		lblRegister = new JLabel("Receptionist Register");
		lblRegister.setForeground(Color.WHITE);
		lblRegister.setFont(new Font("Magneto", Font.BOLD, 23));
		lblRegister.setBounds(525, 60, 300, 27);
		frm.add(lblRegister);
		...
		js = new JScrollPane();
		js.setBounds(790, 180, 480, 415);
		tabview.setFont(f);
		tabview.setForeground(Color.blue);
		tabview.setBackground(Color.WHITE);
		Object[] columns = { "Id", "NAME", "DATE OF BIRTH", "GENDER", "MOBILE NO", "ADDRESS" };
		...
		tabview.addMouseListener(new MouseAdapter()
		{
			...
			public void mouseClicked(MouseEvent e)
			{
				...
				txtMob_No.setText(model.getValueAt(i, 4).toString());
				txtEmailId.setText(model.getValueAt(i, 5).toString());
				txtFull_Address.setText(model.getValueAt(i, 6).toString());
				txtName.grabFocus();
			}
		});
	...
	{
		String sqlqry = "SELECT ID,NAME,DOB,GENDER,MOBILE_NO,EMAILID,ADDRESS FROM RECEPTIONIST_DETAILS";
		...
		if (e.getSource().equals(btnNEW))
		{
			...
			else if (e.getActionCommand().equalsIgnoreCase("SAVE"))
			{
				...
				if (frmvalidate()) 
				{
					...
					if (ro.rec_userValidate())
					{
						if (strongPassword(txtPASS.getText())) 
						{
							if (ro.rec_saverecord() == 1) 
							{
