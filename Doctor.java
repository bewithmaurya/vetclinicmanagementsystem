...
class Doc extends ValidateForm implements ActionListener 
{
	public JFrame frm;
	public PreparedStatement ps;
	public ResultSet rs;
	public Connection conn;
	public JButton btnNEW, btnUploade, btnEDIT, btnCLAR;
	public DefaultTableModel jtable;
	public JTable tabview;
	public JLabel lblId, lbluser, lblsAnswer, lblpass, lblconfpass, lblSearch, barlabel, lblVeterinaryManagementSystem, lblsQueston, lblDoctorRegister, lblQualification, lblEmailId, lblSpez, lblName, lblDOB, lblGender,lblFull_Address, lblMob_No, lblimg;
	public JTextField txtId, txtUser_Id, txtsAnswer, txtName, txtimg, txtDOB, txtSearch, txtQualification, txtMob_No, txtEmailId, txtSpez;
	...
	Doc() 
	{
		...
		lblVeterinaryManagementSystem.setBounds(320, 12, 650, 30);
		...
		lblSpez = new JLabel("SPECILLIZATION");
		lblSpez.setBounds(20, 290, 100, 30);
		lblSpez.setFont(f);
		lblSpez.setForeground(Color.BLUE);
		frm.add(lblSpez);

		txtSpez = new JTextField();
		txtSpez.setBounds(150, 290, 220, 30);
		txtSpez.setFont(f);
		txtSpez.setBackground(Color.WHITE);
		txtSpez.setForeground(Color.BLACK);
		txtSpez.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		frm.add(txtSpez);
		...
		btnCLAR = new JButton("CLEAR");
		btnCLAR.setBounds(535, 530, 235, 35);
		btnCLAR.setFont(f);
		btnCLAR.setForeground(Color.BLUE);
		btnCLAR.setBackground(hexa1);
		btnCLAR.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		btnCLAR.addActionListener(this);
		frm.add(btnCLAR);
		...
		tabview.addMouseListener(new MouseAdapter() 
		{
			...
			public void mouseClicked(MouseEvent e)
			{
				...
				txtSpez.setText(model.getValueAt(i, 5).toString());
	...
	{
		String sqlqry = "SELECT ID,NAME,DOB,GENDER,QUALIFICATION,SPECIALIZATION,MOBILE_NO,EMAILID,ADDRESS FROM DOCTOR_DETAILS";
	...
	public void actionPerformed(ActionEvent e)
	{
		if (e.getSource().equals(btnNEW))
		{
			...
			else if (e.getActionCommand().equalsIgnoreCase("SAVE"))
			{
				...
				if (frmvalidate())
				{
					RegisterOperation ro = new RegisterOperation(txtId.getText(), txtName.getText().toUpperCase(),ddate, cmbGender.getSelectedItem().toString().toUpperCase(), txtQualification.getText().toUpperCase(), txtSpez.getText().toUpperCase(), txtMob_No.getText(), txtEmailId.getText().toLowerCase(), txtFull_Address.getText().toUpperCase(), txtUser_Id.getText(), cmbSecQueston.getSelectedItem().toString(), txtsAnswer.getText(), txtPASS.getText());
					if (ro.doc_userValidate()) 
					{
						if (strongPassword(txtPASS.getText()))
						{
							if (ro.doc_saverecord() == 1) 
							{
		...
		else if (e.getSource().equals(btnEDIT)) 
		{
			...
			else if (e.getActionCommand().equalsIgnoreCase("UPDATE")) 
			{
				...
				{
					RegisterOperation ro = new RegisterOperation(txtId.getText().toUpperCase(), txtName.getText().toUpperCase(), ddate, cmbGender.getSelectedItem().toString().toUpperCase(), txtQualification.getText().toUpperCase(), txtSpez.getText().toUpperCase(), txtMob_No.getText(), txtEmailId.getText().toLowerCase(), txtFull_Address.getText().toUpperCase());
					if (ro.doc_updatebyadmin() == 1) 
					{
		...
		else if (e.getSource().equals(btnCLAR)) 
		{
			clear();
		}
	}
	...
	public boolean frmvalidateforupdate() 
	{
		...
		else if (ValidateForm.validateAnyString(txtSpez.getText()) == false) 
		{
